...
import { SafetyReportReporter } from './safety-report-reporter.js';
...
<body>
  ...
  async writeOutputs(analysisResults, validationResults, additionalMeta = {}) {
    ...
    // Safety Telemetry Report (canonical contract artifact)
    if (this.config?.reporting?.safetyReport?.enabled) {
      const safetyReporter = new SafetyReportReporter(this.config, this.logger);
      const report = safetyReporter.assemble({
        analysisResults,
        validationResults,
        metadata,
        piiSummary: metadata?.piiSummary
      });

      const formats = this.config.reporting.safetyReport.formats || ['json'];

      if (formats.includes('json')) {
        const fp = await safetyReporter.writeJson(dir, report);
        outputs.files.safety_report = fp;
      }

      if (formats.includes('md')) {
        const fp = await safetyReporter.writeMarkdown(dir, report);
        outputs.files.safety_report_md = fp;
      }
    }
